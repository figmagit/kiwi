...
impl<'a> ByteBuffer<'a> {
    ...
    pub fn read_var_uint64(&mut self) -> Result<u64, ()> {
        ...
        loop {
            let byte = self.read_byte()?;
            if byte_count < 8 {
                result |= ((byte & 127) as u64) << (byte_count * 7);
            } else {
                result |= (byte as u64) << (byte_count * 7);
                break;
            }
            byte_count += 1;
...
#[test]
fn read_bool() {
    let data = |bytes| ByteBuffer::new(bytes).read_bool();
    assert_eq!(data(&[]), Err(()));
    assert_eq!(data(&[0]), Ok(false));
    assert_eq!(data(&[1]), Ok(true));
    assert_eq!(data(&[2]), Err(()));
}

#[test]
fn read_byte() {
    let data = |bytes| ByteBuffer::new(bytes).read_byte();
    assert_eq!(data(&[]), Err(()));
    assert_eq!(data(&[0]), Ok(0));
    assert_eq!(data(&[1]), Ok(1));
    assert_eq!(data(&[254]), Ok(254));
    assert_eq!(data(&[255]), Ok(255));
}

#[test]
fn read_bytes() {
    let data = |bytes, len| ByteBuffer::new(bytes).read_bytes(len);
    assert_eq!(data(&[], 0), Ok(vec![].as_slice()));
    assert_eq!(data(&[], 1), Err(()));
    assert_eq!(data(&[0], 0), Ok(vec![].as_slice()));
    assert_eq!(data(&[0], 1), Ok(vec![0].as_slice()));
    assert_eq!(data(&[0], 2), Err(()));
...
#[test]
fn read_var_int() {
    let data = |bytes| ByteBuffer::new(bytes).read_var_int();
    assert_eq!(data(&[]), Err(()));
    assert_eq!(data(&[0]), Ok(0));
    assert_eq!(data(&[1]), Ok(-1));
    assert_eq!(data(&[2]), Ok(1));
    assert_eq!(data(&[3]), Ok(-2));
    assert_eq!(data(&[4]), Ok(2));
    assert_eq!(data(&[127]), Ok(-64));
    assert_eq!(data(&[128]), Err(()));
    assert_eq!(data(&[128, 0]), Ok(0));
    assert_eq!(data(&[128, 1]), Ok(64));
    assert_eq!(data(&[128, 2]), Ok(128));
    assert_eq!(data(&[129, 0]), Ok(-1));
    assert_eq!(data(&[129, 1]), Ok(-65));
    assert_eq!(data(&[129, 2]), Ok(-129));
    assert_eq!(data(&[253, 255, 7]), Ok(-65535));
    assert_eq!(data(&[254, 255, 7]), Ok(65535));
    assert_eq!(data(&[253, 255, 255, 255, 15]), Ok(-2147483647));
    assert_eq!(data(&[254, 255, 255, 255, 15]), Ok(2147483647));
    assert_eq!(data(&[255, 255, 255, 255, 15]), Ok(-2147483648));
}

#[test]
fn read_var_uint() {
    let data = |bytes| ByteBuffer::new(bytes).read_var_uint();
    assert_eq!(data(&[]), Err(()));
    assert_eq!(data(&[0]), Ok(0));
    assert_eq!(data(&[1]), Ok(1));
    assert_eq!(data(&[2]), Ok(2));
    assert_eq!(data(&[3]), Ok(3));
    assert_eq!(data(&[4]), Ok(4));
    assert_eq!(data(&[127]), Ok(127));
    assert_eq!(data(&[128]), Err(()));
    assert_eq!(data(&[128, 0]), Ok(0));
    assert_eq!(data(&[128, 1]), Ok(128));
    assert_eq!(data(&[128, 2]), Ok(256));
    assert_eq!(data(&[129, 0]), Ok(1));
    assert_eq!(data(&[129, 1]), Ok(129));
    assert_eq!(data(&[129, 2]), Ok(257));
    assert_eq!(data(&[253, 255, 7]), Ok(131069));
    assert_eq!(data(&[254, 255, 7]), Ok(131070));
    assert_eq!(data(&[253, 255, 255, 255, 15]), Ok(4294967293));
    assert_eq!(data(&[254, 255, 255, 255, 15]), Ok(4294967294));
    assert_eq!(data(&[255, 255, 255, 255, 15]), Ok(4294967295));
}

#[test]
fn read_var_uint64() {
    let data = |bytes| ByteBuffer::new(bytes).read_var_uint64();
    assert_eq!(data(&[]), Err(()));
    assert_eq!(data(&[0]), Ok(0));
    assert_eq!(data(&[1]), Ok(1));
    assert_eq!(data(&[2]), Ok(2));
    ...
    assert_eq!(
        ...
        Ok(18446744073709551615_u64)
    );
}

#[test]
fn read_var_float() {
    let data = |bytes| ByteBuffer::new(bytes).read_var_float();
    assert_eq!(data(&[]), Err(()));
    assert_eq!(data(&[0]), Ok(0.0));
    assert_eq!(data(&[133, 242, 210, 237]), Ok(123.456));
    assert_eq!(data(&[133, 243, 210, 237]), Ok(-123.456));
    assert_eq!(data(&[254, 255, 255, 255]), Ok(f32::MIN));
    assert_eq!(data(&[254, 254, 255, 255]), Ok(f32::MAX));
    assert_eq!(data(&[1, 1, 0, 0]), Ok(-f32::MIN_POSITIVE));
    assert_eq!(data(&[1, 0, 0, 0]), Ok(f32::MIN_POSITIVE));
    assert_eq!(data(&[255, 1, 0, 0]), Ok(f32::NEG_INFINITY));
    assert_eq!(data(&[255, 0, 0, 0]), Ok(f32::INFINITY));
    assert_eq!(data(&[255, 0, 0, 128]).map(|f| f.is_nan()), Ok(true));
}

#[test]
fn read_string() {
    let data = |bytes| ByteBuffer::new(bytes).read_string();
    assert_eq!(data(&[]), Err(()));
    assert_eq!(data(&[0]), Ok(Cow::Borrowed("")));
    assert_eq!(data(&[97]), Err(()));
    assert_eq!(data(&[97, 0]), Ok(Cow::Borrowed("a")));
    assert_eq!(data(&[97, 98, 99, 0]), Ok(Cow::Borrowed("abc")));
    assert_eq!(data(&[240, 159, 141, 149, 0]), Ok(Cow::Borrowed("🍕")));
    assert_eq!(
        data(&[97, 237, 160, 188, 99, 0]),
...
fn value_basic() {
    let value = Value::Array(vec![
        ...
        Value::Int(-1),
        Value::UInt(1),
        Value::Int64(-8589934592_i64),
        Value::UInt64(8589934592_u64),
        Value::Float(0.5),
    ...
    assert_eq!(value[4], Value::Int64(-8589934592_i64));
    assert_eq!(value[5], Value::UInt64(8589934592_u64));
    assert_eq!(value[6], Value::Float(0.5));
    assert_eq!(value[7], Value::String("abc".to_owned()));
    assert_eq!(value[8], Value::Enum("Foo", "FOO"));
    assert_eq!(
        value[9],
        Value::Object("Obj", {
            let mut map = HashMap::new();
    ...
    assert_eq!(value[4].as_int64(), -8589934592_i64);
    assert_eq!(value[5].as_uint64(), 8589934592_u64);
    assert_eq!(value[6].as_float(), 0.5);
    assert_eq!(value[7].as_string(), "abc");
    ...
    assert_eq!(
        format!("{:?}", value),
        "[true, 255, -1, 1, -8589934592, 8589934592, 0.5, \"abc\", Foo::FOO, Obj {key1: \"value1\", key2: \"value2\"}]"
    );
}
...
fn value_encode_and_decode() {
    ...
    assert_eq!(
        Value::decode(&schema, TYPE_INT64, &[180, 139, 176, 153, 9]),
        Ok(Value::Int64(1234567898_i64))
    );
    assert_eq!(
        Value::decode(&schema, TYPE_UINT64, &[218, 133, 216, 204, 4]),
        Ok(Value::UInt64(1234567898_u64))
    );
    assert_eq!(
        Value::decode(
            &schema,
        ...
        Ok(Value::Int64(-0x8000000000000000_i64))
    );
    assert_eq!(
        Value::decode(
            &schema,
        ...
        Ok(Value::UInt64(0xFFFFFFFFFFFFFFFF_u64))
    );
    assert_eq!(
    ...
    assert_eq!(
        Value::Int64(1234567898_i64).encode(&schema),
        [180, 139, 176, 153, 9]
    );
    assert_eq!(
        Value::UInt64(1234567898_u64).encode(&schema),
        [218, 133, 216, 204, 4]
    );
    assert_eq!(
        Value::Int64(-0x8000000000000000_i64).encode(&schema),
        [255, 255, 255, 255, 255, 255, 255, 255, 255]
    );
    assert_eq!(
        Value::UInt64(0xFFFFFFFFFFFFFFFF_u64).encode(&schema),
        [255, 255, 255, 255, 255, 255, 255, 255, 255]
    );
