  ...
  var x = bb.readVarUint();
    ...
    case 2:
      ...
      if (!!optDedupCache) {
        ...
        for (var i = 0; i < length; i++) this["hashDuplicatedMessage"](hasher);
        arrayDedupKey = { hash: hasher.value(), data: hasher.windowToUint8Array(), defName: "DuplicatedMessage[]" };
        var cachedResult = optDedupCache.get(arrayDedupKey, null);
        if (cachedResult !== null) {
          bb.extendWindowTo(hasher.bb());
          values = cachedResult;
        }
    ...
    case 3:
      ...
      if (!!optDedupCache) {
        ...
        var hasher = new this.Hasher(bb.getNewWindowFromRight());
        var length = hasher.readVarUint();
        for (var i = 0; i < length; i++) this["hashDuplicatedStruct"](hasher);
        arrayDedupKey = { hash: hasher.value(), data: hasher.windowToUint8Array(), defName: "DuplicatedStruct[]" };
        var cachedResult = optDedupCache.get(arrayDedupKey, null);
        if (cachedResult !== null) {
          bb.extendWindowTo(hasher.bb());
          values = cachedResult;
