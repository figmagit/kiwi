...
private:
  kiwi::WriteBuffer &_wb;
public:
  Writer(kiwi::WriteBuffer &wb) : _wb(wb) {}
...
bool parseSortedStruct(kiwi::ByteBuffer &bb, Visitor &visitor) {
  ...
  bool a1;
  if (!bb.readBool(a1)) return false;
  ...
  bool a2;
  if (!bb.readBool(a2)) return false;
  ...
  while (_a3_count-- > 0) {
    bool a3;
    if (!bb.readBool(a3)) return false;
...
void Writer::visitEnumStruct_x(Enum x) {
  _wb.writeVarUint(static_cast<uint32_t>(x));
}

void Writer::visitEnumStruct_y_count(uint32_t size) {
  _wb.writeVarUint(size);
}

void Writer::visitEnumStruct_y_element(Enum y) {
  _wb.writeVarUint(static_cast<uint32_t>(y));
}
...
void Writer::visitNestedStruct(uint32_t a, uint32_t b_x, uint32_t b_y, uint32_t c) {
  _wb.writeVarUint(a);
  _wb.writeVarUint(b_x);
  _wb.writeVarUint(b_y);
  _wb.writeVarUint(c);
}
