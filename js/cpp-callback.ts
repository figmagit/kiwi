...
function argumentForField(definitions: { [name: string]: Definition }, type: string | null, name: string): Argument | null {
  switch (type) {
    ...
    case 'byte':
      return { type: 'uint8_t ', name: name }
    case 'int':
      return { type: 'int32_t ', name: name }
    case 'uint':
      return { type: 'uint32_t ', name: name }
    case 'int64':
      return { type: 'int64_t ', name: name }
    case 'uint64':
      return { type: 'uint64_t ', name: name }
    case 'float':
      return { type: 'float ', name: name }
    case 'string':
      return { type: 'const char *', name: name }
    default: {
...
function argToWrite(arg: Argument): string {
  switch (arg.type) {
    case 'bool ':
      return `_wb.writeBool(${arg.name})`
    case 'uint8_t ':
      return `_wb.writeByte(${arg.name})`
    case 'int32_t ':
      return `_wb.writeVarInt(${arg.name})`
    case 'uint32_t ':
      return `_wb.writeVarUint(${arg.name})`
    case 'int64_t ':
      return `_wb.writeVarInt64(${arg.name})`
    case 'uint64_t ':
      return `_wb.writeVarUint64(${arg.name})`
    case 'float ':
      return `_wb.writeVarFloat(${arg.name})`
    case 'const char *':
      return `_wb.writeString(${arg.name})`
    default:
      return `_wb.writeVarUint(static_cast<uint32_t>(${arg.name}))`
  }
}
...
export function compileSchemaCallbackCPP(schema: Schema): string {
  ...
  for (let pass = 0; pass < 2; pass++) {
    ...
    if (pass === 0) {
      ...
      cpp.push('private:')
      cpp.push('  kiwi::WriteBuffer &_wb;')
      cpp.push('public:')
      cpp.push('  Writer(kiwi::WriteBuffer &wb) : _wb(wb) {}')
      suffix = ') override;'
    }
      ...
      for (let j = 0; j < definition.fields.length; j++) {
        ...
        if (field.isArray) {
          ...
          if (definition.kind === 'MESSAGE') {
            cpp.push(`  _wb.writeVarUint(${field.value});`)
          }
          cpp.push('  _wb.writeVarUint(size);')
          cpp.push('}')
          cpp.push('')
          name += '_element'
        }
