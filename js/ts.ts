...
export function compileSchemaTypeScript(schema: Schema): string {
  ...
  lines.push(indent + 'export type DedupCache = any;')
  lines.push('')

  lines.push(indent + 'export interface Schema {')
  lines.push(indent + '  makeDedupCache(): DedupCache;')
  for (var i = 0; i < schema.definitions.length; i++) {
    var definition = schema.definitions[i]
    ...
    if (definition.kind === 'ENUM') {
      ...
      lines.push(indent + '  decode' + definition.name + '(buffer: Uint8Array, optionalDedupCache?: DedupCache): ' + definition.name + ';')
    }
  }
