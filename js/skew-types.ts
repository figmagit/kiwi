...
export function compileSchemaSkewTypes(schema: Schema): string {
  ...
  lines.push(indent + 'type DedupCache = dynamic')
  lines.push('')

  lines.push(indent + '@import')
  lines.push(indent + 'class Schema {')
  lines.push(indent + '  const makeDedupCache fn() DedupCache')
  ...
  for (var i = 0; i < schema.definitions.length; i++) {
    ...
    if (definition.kind === 'ENUM') {
      ...
      lines.push(indent + '  def decode' + definition.name + '(buffer Uint8Array, optionalDedupCache DedupCache) ' + definition.name)
    }
  }
