    ...
    for (let i = 0; i < definition.fields.length; i++) {
        ...
        switch (field.type) {
            ...
            default: {
                ...
                if (!type) {
                    util_1.error("Invalid type " +
                        util_1.quote(field.type) +
                        " for field " +
                        util_1.quote(field.name), field.line, field.column);
                }
    ...
    for (let j = 0; j < definition.fields.length; j++) {
        ...
        if (definition.kind === "STRUCT") {
            ...
            lines.push("    throw new Error(" +
                util_1.quote("Missing required field " + util_1.quote(field.name)) +
                ");");
        }
        lines.push("  }");
    }
...
function convertSchema(schema) {
    if (typeof schema === "string") {
        return parser_1.parseSchema(schema);
    }
    if (schema instanceof Uint8Array || schema instanceof bb_1.ByteBuffer) {
        return binary_1.decodeBinarySchema(schema);
    }
    return schema;
}
    ...
    for (let i = 0; i < schema.definitions.length; i++) {
        ...
        switch (definition.kind) {
            ...
            case "MESSAGE": {
                ...
                js.push(name +
                    "[" +
                    util_1.quote("decode" + definition.name) +
                    "] = " +
                ...
                js.push(name +
                    "[" +
                    util_1.quote("encode" + definition.name) +
                    "] = " +
